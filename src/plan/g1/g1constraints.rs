//pub use ::plan::plan_constraints::{NEEDS_LOG_BIT_IN_HEADER, NEEDS_LOG_BIT_IN_HEADER_NUM};
pub use ::plan::plan_constraints::*;
use policy::region::BYTES_IN_REGION;

pub const MOVES_OBJECTS: bool = true;
pub const GC_HEADER_BITS: usize = 4;
pub const GC_HEADER_WORDS: usize = 1;
pub const NUM_SPECIALIZED_SCANS: usize = 1;
pub const MAX_NON_LOS_COPY_BYTES: usize = BYTES_IN_REGION / 2;
pub const NEEDS_CONCURRENT_WORKERS: bool = super::ENABLE_CONCURRENT_MARKING;
pub const NEEDS_LOG_BIT_IN_HEADER: bool = false;
pub const NEEDS_LOG_BIT_IN_HEADER_NUM: usize = 2;