...
impl<VM: VMBinding> Plan for GenImmix<VM> {
    ...
    fn schedule_collection(&'static self, scheduler: &GCWorkScheduler<Self::VM>) {
        ...
        if !is_full_heap {
            debug!("Nursery GC");
            self.common()
                .schedule_common::<GenImmixNurseryGCWorkContext>(
                    self,
                    &GENIMMIX_CONSTRAINTS,
                    scheduler,
                );
            ...
            self.common()
                .schedule_common::<GenImmixMatureGCWorkContext<{ TraceKind::Defrag }>>(
                    self,
                    &GENIMMIX_CONSTRAINTS,
                    scheduler,
                );
            ...
            self.common()
                .schedule_common::<GenImmixMatureGCWorkContext<{ TraceKind::Fast }>>(
                    self,
                    &GENIMMIX_CONSTRAINTS,
                    scheduler,
                );
