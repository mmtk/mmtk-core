...
use crate::{mmtk::MMTK, policy::immortalspace::ImmortalSpace};
...
pub struct GenCopy<VM: VMBinding> {
    ...
    pub hi: AtomicBool,
    pub copyspace0: ImmortalSpace<VM>,
...
impl<VM: VMBinding> Plan for GenCopy<VM> {
    ...
    fn get_collection_reserve(&self) -> usize {
        self.nursery.reserved_pages() + self.mature().reserved_pages()
    }

    fn get_pages_used(&self) -> usize {
        self.nursery.reserved_pages()
            + self.mature().reserved_pages()
...
impl<VM: VMBinding> GenCopy<VM> {
    ...
    pub fn mature(&self) -> &ImmortalSpace<VM> {
        &self.copyspace0
    }
