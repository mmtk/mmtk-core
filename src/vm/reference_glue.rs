...
pub trait ReferenceGlue<VM: VMBinding> {
    ...
    /// For reference types, if the referent is changed during GC, the reference
    /// will be added to a queue, and MMTk will call this method to inform
    /// the VM aobut the changes for those references.
    /// Note that this method is called for each type of weak references during GC, and
    /// the references slice will be cleared once this call is returned. That means
