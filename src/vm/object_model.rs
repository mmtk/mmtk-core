...
pub trait ObjectModel<VM: VMBinding> {
    ...
    /// The temporary VM-specific data generated by `attempt_to_forward` which is used when
    /// finishing or reverting the forwarding attempt.  The VM binding can customize the type as
    /// it needs, and usually uses the data to hold the old value of type tag which can be
    /// overwritten to represent forwarding states.
    ...
    /// Revert the forwarding state of `object` to the state before `attempt_to_forward` was
    ...
    ///
    /// If the object is forwarded, return the forwarding pointer; if the state is reverted back to
