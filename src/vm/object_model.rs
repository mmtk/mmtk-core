...
pub trait ObjectModel<VM: VMBinding> {
    ...
    /// A global per-field 1-bit metadata used by LXR's field barrier to record whether a field has
    /// already been logged (recorded) for the current GC. It is generally located in side metadata,
    /// with one bit per field-sized slot rather than one bit per object as with
    /// [`GLOBAL_LOG_BIT_SPEC`](crate::vm::ObjectModel::GLOBAL_LOG_BIT_SPEC).
...
pub mod specs {
    ...
    macro_rules! define_vm_metadata_spec {
        ($(#[$outer:meta])*$spec_name: ident, $is_global: expr, $log_num_bits: expr, $side_min_obj_size: expr) => {
            $(#[$outer])*
            /// A newtype wrapper around [`MetadataSpec`] that identifies this particular per-object
            /// metadata (e.g. whether it is in the header or on the side, and where). Generated by the
            /// `define_vm_metadata_spec` macro for each metadata kind declared on
            ...
            impl $spec_name {
                ...
                /// Like [`side_first`](Self::side_first), but for use with compressed pointers
                /// ([`crate::vm::ObjectModel::COMPRESSED_PTR_ENABLED`]): the region size is one bit
                /// narrower to match the reduced field/pointer width, so this declares the first side
                /// metadata of its kind (global or local) sized for a compressed-pointer heap.
