...
pub fn interior_poiner_in_normal_object() {
    ...
    with_mockvm(
        ...
        || {
            ...
            let mut test_obj = || {
                let addr = memory_manager::alloc(&mut fixture.mutator, OBJECT_SIZE, 8, 0, AllocationSemantics::Default);
                assert!(!addr.is_zero());

                let obj = MockVM::address_to_ref(addr);
                println!("start = {}, end = {}, obj = {}, in-obj addr = {}", addr, addr + OBJECT_SIZE, obj, obj.to_address::<MockVM>());
                memory_manager::post_alloc(&mut fixture.mutator, obj, OBJECT_SIZE, AllocationSemantics::Default);

                let test_internal_ptr = |ptr: Address| {
                    if ptr >= addr + OBJECT_SIZE {
                        println!("ptr = {}, not intenral pointer", ptr);
                        ...
                        println!("ptr = {}, intenral pointer", ptr);
                        // is internal pointer
                        let base_ref = crate::memory_manager::find_object_from_internal_pointer::<MockVM>(ptr, usize::MAX);
