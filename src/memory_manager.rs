...
pub fn alloc<VM: VMBinding>(
    ...
    debug_assert!(
        ...
        "Size {} is larger than allowed min object size {}",
        size,
        MIN_OBJECT_SIZE
    );
    // Assert alignment
    debug_assert!(
        align >= VM::MIN_ALIGNMENT,
        "Alignment {} is smaller than specified VM::MIN_ALIGNMENT {}",
        align,
        VM::MIN_ALIGNMENT
    );
    debug_assert!(
        align <= VM::MAX_ALIGNMENT,
        "Alignment {} is larger than the specified VM::MAX_ALIGNMENT {}",
        align,
        VM::MAX_ALIGNMENT
    );
    // Assert offset
    debug_assert!(
        VM::USE_ALLOCATION_OFFSET || offset == 0,
        "Offset {} is used when VM specifies VM::USE_ALLOCATION_OFFSET = false",
        offset
    );
    ...
    debug_assert!(
        ...
        "Returned address {} (+ offset {}) does not satisfy alignment {}",
        addr,
        offset,
        align
    );
    debug_assert!(
        (addr + size).is_aligned_to(VM::ALLOC_END_ALIGNMENT),
        "End of allocation {} ({} + {}) does not satisfy VM::ALLOC_END_ALIGNMENT {}",
        addr + size,
        addr,
        size,
