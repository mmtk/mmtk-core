...
#[cfg(feature = "gencopy_sanity_gc")]
const META_DATA_PAGES_PER_REGION: usize = max(
    CARD_META_PAGES_PER_REGION,
    <MarkBitMap as PerChunkMetadata>::METADATA_PAGES_PER_CHUNK,
);
...
impl<VM: VMBinding> Space<VM> for CopySpace<VM> {
    ...
    fn grow_space(&self, start: Address, bytes: usize, new_chunk: bool) {
        ...
        #[cfg(feature = "gencopy_sanity_gc")]
        {
            ...
            self.common()
                .mmapper
                .ensure_mapped(start, bytes >> LOG_BYTES_IN_PAGE);
...
impl<VM: VMBinding> CopySpace<VM> {
    ...
    pub fn trace_mark_object(
        ...
        let mark_bitmap = MarkBitMap::of(addr);
        {
            let mark_tables = self.mark_tables.lock().unwrap();
            debug_assert!(
                mark_tables.contains(&mark_bitmap),
                "Invalid chunk {:?}",
                crate::util::conversions::chunk_align_down(addr)
            );
        }
