...
use crate::util::alloc::allocator::estimate_maximum_aligned_size;
...
impl<VM: VMBinding> Allocator<VM> for ImmixAllocator<VM> {
    ...
    fn alloc(&mut self, size: usize, align: usize, offset: usize) -> Address {
        ...
        if new_cursor > self.limit {
            ...
            if estimate_maximum_aligned_size::<VM>(size, align) > Line::BYTES {
...
impl<VM: VMBinding> ImmixAllocator<VM> {
    ...
    fn require_new_block(&mut self, size: usize, align: usize, offset: usize) -> bool {
        ...
        if insufficient_space && estimate_maximum_aligned_size::<VM>(size, align) > Line::BYTES {
            let start = align_allocation_no_fill::<VM>(self.large_cursor, align, offset);
            let end = start + size;
