...
pub trait GenericFreeList: Sized {
  ...
  fn alloc(&mut self, size: i32) -> i32 {
    ...
    let mut s = 0;
    while ({ unit = self.get_next(unit); unit != self.head() })
      && ({ s = self.get_size(unit); s < size }) {}
    // loop {
  ...
  fn free(&mut self, unit: i32, return_coalesced_size: bool) -> i32 {
    ...
    let start = if self.is_coalescable(unit) && self.get_free(left) { left } else { unit };
    let right = self.get_right(unit);
    let end = if self.is_coalescable(right) && self.get_free(right) { right } else { unit };
    if start != end {
